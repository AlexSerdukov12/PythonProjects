...
# במידה ואין מטריצה הופכית לעשות פירוק  ל LU אחרת פתרון בדרךש של גאוס A|b =ווקטור x
try:
    inv_A =inverse(A)
    n=len(A)
    print('go to vector x = A^-1 |b')
    gaussy(A, b, n)
except ValueError:
    print('go to LU')
    ...
    try:
         U_minus_1 = inverse(upper_matrix)
    except ValueError:
        print('there is no inverse for U for matrix,and there is no sultion for X')
        sys.exit()

    try:
         L_minus_1 = inverse(lower_matrix)
    except ValueError:
        print('there is no inverse for L for matrix,and there is no sultion for X')
        sys.exit()

    L_1b = multiply_matrix_by_vector(U_minus_1, b)  # L^-1*b vector
    Y = multiply_matrix_by_vector(L_minus_1, b)  ## vector Y
    x_vector = multiply_matrix_by_vector(U_minus_1, Y)
    print('vecor x = ', x_vector)
